...
from mari_kit.connectors._shared import json_response, send
from mari_kit.connectors.protocol import ValidationResult, configured_source_id
from mari_kit.errors import PermanentFailure
from mari_kit.http import HttpRequest, HttpTransport
from mari_kit.types import (
    KnowledgeDocument,
    PollPage,
    PollRequest,
    content_revision,
...
def _headers(config: GitLabConfig) -> dict[str, str]:
    return {"PRIVATE-TOKEN": config.token.strip(), "User-Agent": "mari-kit"}
